...
const ChatContainer = () => {
    ...
    const { messages, selectedUser, setSelectedUser, sendMessage, 
        getMessages,typingStatus} = useContext(ChatContext)
    ...
    <div className='h-full overflow-scroll relative backdrop-blur-lg'>
      ...
      <div className='flex flex-col h-[calc(100%-120px)] overflow-y-scroll p-3 pb-6'>
        {messages.map((msg, index)=>(
            <div 
            onClick={()=>clickedMessages?setClickedMessages(false):setClickedMessages(true)}
            key={index} className={`flex items-end gap-2 justify-end ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
                ...
                <span className={`text-[10px] ${msg.seen ? 'text-grey-400' : 'text-gray-500'}`}>
                    {msg.seen ? "Seen" : "Sent"}
                </span>
                )}
...
{/* ------- bottom area ------- */}
   ...
   className='flex flex-col items-center justify-center gap-0 text-gray-500 max-md:hidden ' 
